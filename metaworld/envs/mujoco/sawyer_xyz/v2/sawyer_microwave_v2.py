...
class SawyerMicrowaveEnvV2(SawyerXYZEnv):
    def __init__(self):
        ...
        self.door_angle_idx = self.model.get_joint_qpos_addr('microjoint')
    ...
    @property
    def model_name(self):
        return full_v2_path_for('sawyer_xyz/sawyer_microwave.xml')
    ...
    def observation_space(self):
        ...
        return Box(
            low=0, high=255, shape=img.shape
        )
    ...
    def reset_model(self):
        ...
        self.sim.model.body_pos[self.model.body_name2id('microwave')] = self.obj_init_pos
    ...
    def compute_reward(self, actions, obs):
        theta = self.data.get_joint_qpos('microjoint')

        reward_grab = SawyerMicrowaveEnvV2._reward_grab_effort(actions)
        reward_steps = SawyerMicrowaveEnvV2._reward_pos(obs, theta)
    ...
    def render(self, mode='human'):
        if mode == 'human':
            ...
            return self.sim.render(
                *self._rgb_array_res,
                mode='offscreen',
                camera_name='corner' # topview, gripperPOV, behindGripper, corner, corner2
            )[:, :, ::-1]
        else:
